...
{
    static constexpr auto kDataType = ATScene::AttributeDataType::Float;
    std::vector<float> _data;

public:
    ATFloatAttribute(const NodeID owner, const bool isArray, const AttributeDirection direction) :
        ATAttribute(kDataType, owner, isArray, direction)
    {
        _data.resize(isArray ? 2 : 1);
    }
    ~ATFloatAttribute() override = default;
